    ...
    with logging_redirect_tqdm():
        ...
        with ThreadPoolExecutor(max_workers=20) as executor:
            ...
            for data in pair_data:
                task_objective = data["golden_traj"][0]["content"]
                golden_traj = data["golden_traj"]
                if golden_traj[-1]["role"] != "user":
                    current_traj = cov_to_text(golden_traj[1:-1])
                else:
                    current_traj = cov_to_text(golden_traj[1:-2])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--inst_path",
        type=str,
        default="training_data_construction/prompt/rule_select_inst_alfworld.txt",
        help="Instruction path of selecting rules for ALFWorld.",
    )
    parser.add_argument(
        "--pair_path",
        type=str,
